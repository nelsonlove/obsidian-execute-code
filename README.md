...
If the note has a heading matching the "Docstring heading" setting (default `Docstring`), that heading's section is rendered as a comment block below the generated-file header in every artifact the note tangles — a docstring sourced from your prose. Editing the section re-tangles the files. Code fences inside the docstring section are treated as examples: they appear in the commented docstring but are not tangled as code, unless a fence names an explicit `tangle` destination.
